...
const SCENES: StoryScene[] = [
    ...
    {
        id: 's1-mystery',
        rumiText: (step: number) => step === 0 ? "Hello everyone! I'm Rumi! Today, I'm going to help you understand the magic of Fractions!" : "Are you ready? Let's open this mystery box and see what's inside...",
    ...
    {
        id: 'c1-what-is',
        rumiText: (step: number) => step === 0 ? "Wow, a whole pizza! But wait, if we cut it into equal pieces..." : step === 1 ? "That's a fraction! Look, one slice out of two makes one-half (1/2)." : step === 2 ? "Cutting it again, and it's one-fourth (1/4)!" : "Slice it into 8, and each piece is one-eighth (1/8) of the whole!",
        renderVisual: (step, setStep) => {
            ...
            return (
                <div className="w-full h-full flex flex-col items-center justify-center space-y-12 pt-40 pb-40">
                    ...
                    <button
                        className="px-8 py-3 bg-violet-600 hover:bg-violet-500 text-white rounded-full font-black tracking-wide shadow-[0_0_20px_rgba(124,58,237,0.4)] transition-all uppercase"
    ...
    {
        id: 'c3-comparing',
        rumiText: (step: number) => step === 0 ? "Now let's compare! We have two empty glasses. One is for 1/2 and the other is for 1/4." : step === 1 ? "Do you know which glass will fill more? 1/2 or 1/4? Let's pour the juice and find out!" : "See? The smaller the bottom number, the bigger the portion! Half is way more than a fourth!",
        renderVisual: (step, setStep) => {
                ...
                <div className="w-full h-full flex flex-col items-center justify-center space-y-16 pt-24">
                    <div className="flex justify-center items-end gap-16 h-72 border-b-4 border-slate-700 w-full max-w-2xl px-8 pb-4">
                        ...
                        <div className="flex flex-col items-center gap-4">
                            <span className="text-2xl font-black text-orange-400 bg-orange-400/10 px-4 py-2 rounded-lg border border-orange-400/20 font-mono">1 / 2</span>
                            <motion.div
                        ...
                        {/* 1/4 Glass */}
                        <div className="flex flex-col items-center gap-4">
                            <span className="text-2xl font-black text-orange-400 bg-orange-400/10 px-4 py-2 rounded-lg border border-orange-400/20 font-mono">1 / 4</span>
                            <motion.div
                                className="w-24 border-x-4 border-b-4 border-slate-400/50 rounded-b-xl relative bg-white/5 backdrop-blur-md overflow-hidden shine-effect"
                                initial={{ height: 160, scale: 1 }}
                                animate={{ height: 160, scale: 1 }}
                    ...
                    <div className="h-16 flex items-start justify-center w-full z-10">
                        <AnimatePresence>
                            {showBigger && (
                                ...
                                >
                                    <span className="text-orange-400">1/2</span> &gt; <span className="text-orange-400">1/4</span>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </div>

                    <div className="flex gap-4 z-20 mt-4 h-14 items-center justify-center">
                        {step === 0 && (
    ...
    {
        id: 'c5-sharing',
        rumiText: (step: number) => step === 0 ? "Here comes the ultimate test! I have one big chocolate bar..." : step === 1 ? "And I need to share it equally with my 6 cousins! Let's break it apart." : step === 2 ? "One for you, one for you... spreading them out!" : step === 3 ? "Can you guess what fraction of the chocolate each cousin got?" : "Yay! Every cousin gets exactly 1 out of 6 pieces... which is one-sixth (1/6)!",
        renderVisual: (step, setStep) => {
                ...
                <div className="w-full h-full flex flex-col items-center justify-center space-y-6">
                                        ...
                                        <div className="h-8 mt-2">
                                            {showAnswer && (
                                                <motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: kidId * 0.1, type: "spring" }}>
                                                    <span className="bg-slate-800 text-emerald-400 font-bold px-4 py-1.5 rounded-full text-base border border-emerald-400/20 font-mono drop-shadow-[0_0_10px_rgba(16,185,129,0.3)]">1 / 6</span>
                                                </motion.div>
                                            )}
                                        </div>
                                    </div>
                    ...
                    <div className="h-14 flex items-center justify-center">
                        {step < 4 && (
                            ...
                            >
                                {step === 0 ? "Show Cousins 👶" : step === 1 ? "Break & Share! 🍫" : step === 2 ? "Ask Question 🤔" : "What Fraction? 💡"}
                            </button>
                        )}
                        {step === 4 && (
    ...
    {
        id: 'end',
        rumiText: "Awesome job! Remember, fractions are just equal parts of a whole! You are all Fraction Masters now!",
        renderVisual: (step, setStep) => {
            return (
...
const FractionStoryPage: React.FC = () => {
    ...
    React.useEffect(() => {
        ...
        if (currentRumiAudio) {
            ...
            /* Audio disabled requested by user
            rumiAudioRef.current = new Audio(currentRumiAudio);

            timeoutId = setTimeout(() => {
                if (rumiAudioRef.current) {
                    rumiAudioRef.current.play().catch(e => console.log("Audio play prevented:", e));
                }
            }, 200);
            */
        }
    ...
    React.useEffect(() => {
        ...
        const handleInteraction = () => {
            if (bgAudioRef.current && bgAudioRef.current.paused) {
                bgAudioRef.current.play().catch(e => console.log("BG Audio play prevented:", e));
            }
    ...
    return (
        <div className="w-screen h-screen bg-[#0f172a] flex items-center justify-center p-2 relative overflow-hidden font-sans">
            ...
            <div className="w-full max-w-6xl h-full max-h-[90vh] bg-[#1e293b] rounded-[2rem] shadow-[0_0_80px_rgba(0,0,0,0.8)] border border-slate-700/50 flex flex-col overflow-hidden relative z-10 mx-auto">
                ...
                <div className="h-16 bg-slate-900/50 border-b border-slate-800 flex items-center justify-between px-8 shrink-0 backdrop-blur-sm z-20">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-xl font-black text-white tracking-tight uppercase">Animated Story: Fractions</h1>
                    </div>
                    <div className="flex gap-4 items-center">
                        <span className="text-slate-400 font-bold text-xs uppercase tracking-widest">Scene {sceneIndex + 1} of {SCENES.length}</span>
                ...
                <div className="flex-grow relative bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-slate-800 via-slate-900 to-slate-900 overflow-hidden shadow-inner isolate flex items-center justify-center">
                    ...
                    <div className="absolute bottom-8 right-8 z-30 flex items-center gap-4 bg-slate-900/80 p-3 rounded-full backdrop-blur border border-slate-700 shadow-xl">
                        <button
                            onClick={prevScene}
...
export default FractionStoryPage;
