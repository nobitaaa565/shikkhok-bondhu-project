...
# Instead of parsing, we can just replace what's missing manually.
# Let's add the sections right before `</div>\n      <style>` in EducoreContents
...
if insert_point_search in educore_content:
    ...
    print("Success inserting into EducoreContents!")
else:
